...
__device__ inline int xlns16d_is_zero(xlns16 x)
{
	return (xlns16_abs(x) == xlns16_zero);
}

__device__ inline int xlns16d_is_negative(xlns16 x)
{
	return (xlns16_sign(x) != 0) && !xlns16d_is_zero(x);
}

__device__ inline int xlns16d_is_positive(xlns16 x)
{
	return (xlns16_sign(x) == 0) && !xlns16d_is_zero(x);
}

__device__ inline int xlns16d_gt(xlns16 a, xlns16 b)
{
	return xlns16_canon(a) > xlns16_canon(b);
}

__device__ inline int xlns16d_lt(xlns16 a, xlns16 b)
{
	return xlns16_canon(a) < xlns16_canon(b);
}

__device__ inline int xlns16d_eq(xlns16 a, xlns16 b)
{
	return a == b;
}

__device__ inline int xlns16d_ge(xlns16 a, xlns16 b)
{
	return xlns16_canon(a) >= xlns16_canon(b);
}

__device__ inline int xlns16d_le(xlns16 a, xlns16 b)
{
	return xlns16_canon(a) <= xlns16_canon(b);
}

__device__ inline xlns16 xlns16d_max(xlns16 a, xlns16 b)
{
	return xlns16d_gt(a, b) ? a : b;
}

__device__ inline xlns16 xlns16d_min(xlns16 a, xlns16 b)
{
	return xlns16d_lt(a, b) ? a : b;
}

__device__ inline xlns16 xlns16d_copysign(xlns16 x, xlns16 y)
{
...
__global__ void xlns16d_batch_mul_kernel(const xlns16 *a, const xlns16 *b, xlns16 *c, size_t n)
{
	size_t i = blockIdx.x * blockDim.x + threadIdx.x;
	size_t stride = blockDim.x * gridDim.x;
	for (; i < n; i += stride)
		c[i] = xlns16d_mul(a[i], b[i]);
}

__global__ void xlns16d_batch_add_kernel(const xlns16 *a, const xlns16 *b, xlns16 *c, size_t n)
{
	size_t i = blockIdx.x * blockDim.x + threadIdx.x;
	size_t stride = blockDim.x * gridDim.x;
	for (; i < n; i += stride)
		c[i] = xlns16d_add(a[i], b[i]);
}

__global__ void xlns16d_batch_sub_kernel(const xlns16 *a, const xlns16 *b, xlns16 *c, size_t n)
{
	size_t i = blockIdx.x * blockDim.x + threadIdx.x;
	size_t stride = blockDim.x * gridDim.x;
	for (; i < n; i += stride)
		c[i] = xlns16d_sub(a[i], b[i]);
}

__global__ void xlns16d_batch_div_kernel(const xlns16 *a, const xlns16 *b, xlns16 *c, size_t n)
{
	size_t i = blockIdx.x * blockDim.x + threadIdx.x;
	size_t stride = blockDim.x * gridDim.x;
	for (; i < n; i += stride)
		c[i] = xlns16d_div(a[i], b[i]);
}

__global__ void xlns16d_batch_scale_kernel(const xlns16 *a, xlns16 scalar, xlns16 *c, size_t n)
{
	size_t i = blockIdx.x * blockDim.x + threadIdx.x;
	size_t stride = blockDim.x * gridDim.x;
	for (; i < n; i += stride)
		c[i] = xlns16d_mul(a[i], scalar);
}

__global__ void xlns16d_batch_neg_kernel(const xlns16 *a, xlns16 *c, size_t n)
{
	size_t i = blockIdx.x * blockDim.x + threadIdx.x;
	size_t stride = blockDim.x * gridDim.x;
	for (; i < n; i += stride)
...
__global__ void xlns16d_batch_abs_kernel(const xlns16 *a, xlns16 *c, size_t n)
{
	size_t i = blockIdx.x * blockDim.x + threadIdx.x;
	size_t stride = blockDim.x * gridDim.x;
	for (; i < n; i += stride)
